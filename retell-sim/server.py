        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    "Judge whether the SCENARIO'S goal was achieved AND explicitly confirmed by the agent. "
                    "The valid success depends on the scenario (see the Scenario line). It is ONE of:\n"
                    "  A) BOOKING — appointment booked with an explicit confirmation (date + time stated as booked).\n"
                    "  B) RESCHEDULE — existing appointment moved; agent confirms the NEW date/time.\n"
                    "  C) CANCELLATION — appointment canceled; agent explicitly confirms it was canceled.\n"
                    "  D) TASK CREATION — agent genuinely cannot complete it in-system, collects name + DOB + "
                    "reason, and explicitly confirms a note/task was created for the team.\n\n"
                    "For a Cancel scenario, a confirmed cancellation IS full success (no booking needed). "
                    "For a Reschedule scenario, a confirmed move IS full success.\n\n"
                    "Score 0-100 by the TERMINAL OUTCOME, not effort:\n"
                    "  90-100: The scenario's goal confirmed (booked / rescheduled / canceled / task), smooth, no errors or repetition.\n"
                    "  75-89:  Goal confirmed but with minor friction, OR task created with all required details.\n"
                    "  55-74:  Goal reached only after notable friction, OR task created but missing details.\n"
                    "  30-54:  NO completion — conversation ended without the scenario's goal confirmed.\n"
                    ...
                    "  - No explicit confirmation of the scenario's goal (booking, reschedule, cancellation, or task) → score MUST be < 55.\n"
                    ...
                    "Reply ONLY with JSON: {\"score\": <int>, \"outcome\": \"booking|reschedule|cancel|task|incomplete|failed\", "
