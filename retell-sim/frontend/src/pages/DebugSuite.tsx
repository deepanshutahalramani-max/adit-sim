...
import {
  ...
  MessageSquare, Phone, X, RotateCcw,
} from "lucide-react";
    ...
    <div>
      ...
      <div className="mb-5">
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-1 bg-canvas-sunken rounded-xl p-1 flex-shrink-0 ml-4">
            ...
            >
              <MessageSquare className="w-4 h-4 inline -mt-0.5 mr-1.5" strokeWidth={2} />SMS
            </button>
            <button
            ...
            >
              <Phone className="w-4 h-4 inline -mt-0.5 mr-1.5" strokeWidth={2} />Call
            </button>
          </div>
        </div>
      </div>
            ...
            <div key={s.id} className="flex items-center flex-shrink-0">
              <div className="flex items-center gap-2">
                <div className={clsx(
                  ...
                  "bg-white border-line-strong text-ink-400",
                )}>
                  {isDone ? <Check className="w-3.5 h-3.5" strokeWidth={2.5} /> : s.id}
                </div>
                <span className={clsx(
                  "text-[13px] font-medium hidden sm:block whitespace-nowrap",
                  isActive ? "text-ink-900 font-bold" : isDone ? "text-brand-500" : "text-ink-400",
                )}>{s.label}</span>
              </div>
              {i < STEPS.length - 1 && (
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-[13px] text-red-600 mb-5 flex items-center gap-2">
          ...
          {error}
          <button onClick={() => setError("")} className="ml-auto text-ink-400 hover:text-red-600"><X className="w-3.5 h-3.5" strokeWidth={2} /></button>
        </div>
      )}
            ...
            <>
                ...
                {(["screenshot", "text"] as const).map(m => (
                  <button key={m} onClick={() => setInputMode(m)}
                    className={clsx("px-4 py-2 text-[13px] font-semibold rounded-lg border transition-colors",
                      inputMode === m ? "bg-brand-500 text-white border-brand-500" : "bg-white text-ink-500 border-line hover:border-brand-500",
                    )}>
                    {m === "screenshot" ? "Screenshot" : "Text description"}
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 gap-5 mb-4">
                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] font-bold uppercase tracking-widest text-ink-400 block mb-1.5">
                    ...
                    <textarea value={extraContext} onChange={e => setExtraContext(e.target.value)} rows={3}
                      ...
                      className="w-full border border-line rounded-xl px-4 py-3 text-[13px] text-ink-900 resize-none focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10" />
                  </div>
                </div>
                  ...
                  {inputMode === "screenshot" ? (
                    <>
                      <div onDrop={handleDrop} onDragOver={e => e.preventDefault()} onClick={() => fileRef.current?.click()}
                        className="flex-1 border-[1.5px] border-dashed border-line-strong rounded-xl cursor-pointer hover:border-brand-500 transition-colors flex items-center justify-center bg-canvas min-h-[200px]">
                        {screenshotUrl
                          ...
                          : <div className="text-center p-8">
                              ...
                              <div className="text-[13px] font-medium text-ink-500">Drop screenshot here or click</div>
                              <div className="text-[12px] text-ink-400 mt-1">PNG · JPG · WEBP</div>
                            </div>}
                      </div>
                    ...
                    <textarea value={description} onChange={e => setDescription(e.target.value)}
                      className="flex-1 border border-line rounded-xl px-4 py-3 text-[13px] text-ink-900 resize-none focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 min-h-[200px]"
              ...
              <div className="flex gap-2 mb-5">
                {(["screenshot", "transcript"] as const).map(m => (
                  <button key={m} onClick={() => setCallInputMode(m)}
                    className={clsx("px-4 py-2 text-[13px] font-semibold rounded-lg border transition-colors",
                      callInputMode === m ? "bg-brand-500 text-white border-brand-500" : "bg-white text-ink-500 border-line hover:border-brand-500",
                    )}>
                ...
                <div className="flex flex-col">
                  ...
                  {callInputMode === "screenshot" ? (
                    ...
                    <textarea
                      ...
                      className="flex-1 border border-line rounded-xl px-4 py-3 text-[13px] text-ink-900 resize-none focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/10 min-h-[320px] font-mono"
        ...
        <div>
          <div className="bg-white border border-line rounded-2xl p-6 mb-5 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-4 mb-5">
              <div className="bg-canvas border border-line rounded-xl p-4">
                <div className="text-[10px] font-bold uppercase tracking-widest text-ink-400 mb-2">What Happened</div>
                <textarea value={editedWhatHappened} onChange={e => setEditedWhatHappened(e.target.value)} rows={4}
                  className="w-full bg-transparent text-[13px] text-ink-700 leading-relaxed resize-none focus:outline-none border-b border-dashed border-line-strong focus:border-brand-500" />
              </div>
              ...
              <div>
                <div className="rounded-xl p-4 mb-3" style={{ background: SEV_BG[diagnosis.severity] ?? "#FAFAF8", border: "1px solid #EAEAEA" }}>
                  <div className="text-[10px] font-bold uppercase tracking-widest text-ink-400 mb-1">Severity · Confidence</div>
                  <div className="text-[22px] font-extrabold capitalize" style={{ color: SEV_COLOR[diagnosis.severity] }}>{diagnosis.severity}</div>
                  <div className="text-[12px] text-ink-500">Confidence: {diagnosis.confidence}</div>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setStep("input")}
              className="flex items-center gap-1.5 px-4 py-3 text-[13px] font-semibold text-ink-500 bg-white border border-line rounded-xl hover:border-[#ADADAD] transition-colors">
      ...
      {step === "reproducing" && diagnosis && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="text-[15px] font-semibold text-ink-900 tracking-[-0.01em]">
                ...
                {totalRuns > 0 && <span className="text-[13px] font-normal text-ink-500 ml-2">of {MAX_REPRO_RUNS} max</span>}
              </div>
              <div className="text-[12.5px] text-ink-500 mt-0.5">
          ...
          <div className="bg-canvas border border-line rounded-xl p-4 mb-4">
            <div className="text-[10px] font-bold uppercase tracking-widest text-ink-400 mb-2">Repro Scenario</div>
            ...
            {diagnosis.repro_followups?.map((f, i) => (
              <div key={i} className="text-[13px] text-ink-500 ml-4">↳ {f}</div>
            ))}
          </div>
          ...
          <div className="bg-white border border-line rounded-xl p-4 mb-4">
            <div className="text-[13px] font-semibold text-ink-900 mb-1">
              {mode === "call" ? "Reproduce over a real call" : "Reproduce over real SMS"}
            </div>
            ...
            {realEnv(config.environment) && !(config.environment === "custom" && !config.customNumber) ? (
              <RealRunPanel
                ...
                buttonLabel={mode === "call" ? "Reproduce over real calls (2 runs)" : "Reproduce over real SMS (2 runs)"}
              />
            ) : (
              <div className="text-[12.5px] text-[#B45309] bg-[#FFF7ED] border border-[#FED7AA] rounded-xl px-3 py-2">
              ...
              {mode === "sms" ? (
                ...
                <div key={streamKey}>
                  ...
                  <div className="bg-[#F0F5FF] border border-[#C7D7FD] rounded-xl p-4 mb-3">
                    ...
                    {(diagnosis.repro_followups ?? []).map((f, i) => (
                      <div key={i} className="text-[12.5px] text-ink-500 ml-3 mt-0.5">↳ {f}</div>
                    ))}
      ...
      {step === "confirm_fix" && diagnosis && (
        <div>
          <div className="bg-white border border-line rounded-2xl p-6 mb-5 shadow-sm">
            <div className="text-[14px] font-bold text-ink-900 mb-4">Proposed prompt change</div>
      ...
      {step === "done" && regressionSummary && (
        <div>
          <div className="bg-white border border-line rounded-2xl p-6 mb-5 shadow-sm">
            <div className="flex items-center gap-4 mb-5">
              <div className={clsx(
                ...
                <span className="text-[10px] text-ink-400 mt-0.5">pass rate</span>
              </div>
              <div>
                <div className="text-[16px] font-semibold text-ink-900 tracking-[-0.01em]">
            ...
            <details className="border-t border-line pt-4 mb-3">
              ...
              <pre className="mt-3 bg-canvas-sunken border border-line rounded-xl p-4 text-[12px] text-ink-700 overflow-auto max-h-52 whitespace-pre-wrap font-mono leading-relaxed">
                {modifiedPrompt}
              </pre>
            ...
            {originalPrompt && (
              <details className="border border-line rounded-xl overflow-hidden">
                <summary className="cursor-pointer px-4 py-3 text-[13px] font-semibold text-ink-500 select-none hover:bg-canvas inline-flex items-center gap-1.5">
                  <RotateCcw className="w-3.5 h-3.5" strokeWidth={2} /> Revert to original prompt
                </summary>
                <div className="px-4 pb-4">
                  ...
                  <button onClick={() => copy(originalPrompt, "original")}
                    className="mt-2 flex items-center gap-1.5 text-[12.5px] font-semibold text-ink-500 hover:underline">
          ...
          <button onClick={handleReset}
            className="flex items-center gap-2 text-[13px] font-semibold text-ink-500 bg-white border border-line rounded-xl px-5 py-2.5 hover:border-brand-500 hover:text-brand-500 transition-colors">
            <RefreshCw className="w-3.5 h-3.5" />
...
function LoadingCard({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="bg-white border border-line rounded-2xl p-12 flex flex-col items-center justify-center text-center shadow-sm">
      <div className="w-10 h-10 border-[3px] border-brand-500 border-t-transparent rounded-full animate-spin mb-5" />
      <div className="text-[15px] font-bold text-ink-900 mb-1.5">{title}</div>
      <div className="text-[13px] text-ink-500 max-w-xs leading-relaxed">{subtitle}</div>
    </div>
  );
}
