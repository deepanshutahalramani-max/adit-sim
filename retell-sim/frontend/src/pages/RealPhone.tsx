...
import {
  ...
  triggerReal, setupRealWebhooks, runRealSuite,
  manualStart, manualSend, manualEnd,
} from "../api";
import { RealSessionCard, REAL_TRIGGERS, STATUS_STYLES, STATUS_LABEL, fmtPhone } from "../components/RealSessionCard";

/* ── Constants ─────────────────────────────────────────────────────────────── */

const TRIGGERS = REAL_TRIGGERS;
    ...
    <div className="space-y-5">
      ...
      {activeSessions.length > 0 && (
        <div className="space-y-3">
          ...
          {activeSessions.map(s => <RealSessionCard key={s.session_id} s={s} />)}
        </div>
      )}
      ...
      {recentDone.length > 0 && (
        <div className="space-y-3">
          ...
          {recentDone.map(s => <RealSessionCard key={s.session_id} s={s} compact />)}
        </div>
      )}
    </div>
  );
